...
bool node_arp_send_broadcast_request(node_t *n, interface_t *ointf, ipv4_addr_t *ip_addr) {
  ...
  ether_hdr_set_src_mac(ether_hdr, INTF_MAC(ointf)); // our intf MAC
  ...
  arp_hdr_set_src_mac(arp_hdr, INTF_MAC(ointf));
  arp_hdr_set_src_ip(arp_hdr, INTF_IP(ointf)->value);
...
bool node_arp_recv_broadcast_request_frame(node_t *n, interface_t *iintf, ether_hdr_t *hdr) {
  ...
  if (IPV4_ADDR_PTR_IS_EQUAL(INTF_IP(iintf), &target_ip)) { 
...
bool node_arp_send_reply_frame(node_t *n, interface_t *ointf, ether_hdr_t *in_ether_hdr) {
  ...
  ether_hdr_set_src_mac(out_ether_hdr, INTF_MAC(ointf));
...
bool arp_table_process_reply(arp_table_t *t, arp_hdr_t *hdr, interface_t *intf) {
  ...
  arp_entry_t entry = {0};
  entry.ip_addr = arp_hdr_read_src_ip(hdr); 
  entry.mac_addr = arp_hdr_read_src_mac(hdr);
